...
#
#   This script is used to process the input through Stable Diffusion
#
...
def getInitSettings(): # -DC-
    ...
    print("\n---------------------")
    print("\nEnter a Prompt Key")
    print("(Blank input defaults it to defaultPromptKey)\n")
    print("Prompt Keys:")
    for promptKey in promptKeys:
        print(promptKey)
    print("")
    chosenPromptKey = input("Prompt Key > ")
    if chosenPromptKey == "":
...
def getInvokeCommand(settings):  # command string
    try:
        ...
        command = PROMPT + " -I " + INPUT_FILEPATH + " -o "+OUTPUT_FOLDER+" -s "+str(SAMPLES)+" -W512 -H512 --fit --strength " + str(DENOISING)
        ...
        print("Awaiting source image (run capture_diffusioncraft)")
        return ""


def main_invoke(settings):
    ...
    print("\n\n\n---------------------")
    print("\n* Initializing Invoke AI, be patient...")
    ...
    try:
        main_loop(gen, opt, infile, settings)
    except KeyboardInterrupt:
        print("\ngoodbye!")
...
def main_loop(gen, opt, infile, settings):
    ...
    while not done:
        ...
        try:
            command = getInvokeCommand(settings)
            if command == "":
                continue
            if command == "-quit-":
                return
